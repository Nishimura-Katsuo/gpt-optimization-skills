...
- For an Astra route, use `gpt-6-astra` when that exact ID is listed in the current `spawn_agent` model overrides. If it is not listed, use `gpt-5.6-sol` with the same reasoning effort. Do not attempt an unlisted model or make any other substitution.

## Route selection

Choose by task shape, not by output length or the parent agent's model.

### Astra on xhigh: difficult judgment and ambiguity

Use `gpt-6-astra` on `xhigh` when available, otherwise `gpt-5.6-sol` on `xhigh`, when the subtask needs the highest level of critical thinking, has a very long agentic horizon, contains meaningful ambiguity, is not yet well-defined, or must create or substantially revise a plan. This is the default route for architecture, difficult diagnosis, uncertain tradeoffs, synthesis across conflicting evidence, high-consequence review, and most original planning.
...
task_name: [task name]
agent_type: default
model: gpt-6-astra # use gpt-5.6-sol while Astra is not listed
reasoning_effort: xhigh
fork_turns: none
message: [manually curated instructions]
```

### Astra on low: well-defined short-horizon coordination

Use `gpt-6-astra` on `low` when available, otherwise `gpt-5.6-sol` on `medium`, for short-horizon coordination or integration requiring limited critical thinking when the project, plan, architecture, interfaces, and acceptance criteria are already well-defined. Good uses include coordinating multiple implementation workstreams, sequencing an approved plan, reconciling predetermined outputs, or combining specified steps without reopening settled design decisions.

Do not use this route to invent the plan it is coordinating. Defer a single, tightly bounded implementation slice to Luna/max when it can be handed off completely. Escalate to Astra/xhigh if requirements conflict, important decisions are missing, or the agent must make architectural judgments.
...
Here, `max` is for thoroughness and coverage, not permission to infer missing requirements. If the agent would need to choose among interpretations, resolve a product or architectural tradeoff, or decide what matters without explicit criteria, use Astra/xhigh instead. If a mechanical task encounters unexpected ambiguity, have it report the ambiguity and evidence rather than guessing.
...
For Luna/max, every field should be concrete enough that the agent can proceed without interpreting intent. For Luna implementation, include the exact plan step, affected files or components, expected behavior, and validation. For Astra/medium, the plan and design decisions must already be settled. Otherwise use Astra/xhigh.
